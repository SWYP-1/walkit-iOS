...
struct EmotionSlider: View {
    ...
    var body: some View {
        HStack {
            ...
            ZStack {
                Capsule()
                    .fill(Color(hex: "#D7D9E0"))
                    .frame(width: 20)
                    .padding(.horizontal, 20)
                GeometryReader { geo in
                    let height = geo.size.height
                    let usableHeight = height - (indicatorHeight / 2)
                    let step = height / CGFloat(maxValue - 1)
                    
                    ZStack {
                        Image("EmotionIndicator")
                            .resizable()
                            .scaledToFit()
                            .frame(width: 60, height: indicatorHeight)
                            .offset(y: (emotion.isEmpty ? (usableHeight - CGFloat(value) * step + (step / 2)) : (usableHeight - CGFloat(value) * step)))
                            .gesture(
                                DragGesture()
                                    .onChanged { g in
