...
struct MainTabView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if(showTabBar) {
                HStack {
                    TabBarButton(iconName: "house", title: "홈", isSelected: selection == .HOME) {
                        if(selection == .HOME) {
                            walkViewMoel.goHome()
                        } else {
                            selection = .HOME
                    ...
                    TabBarButton(iconName: "doc.plaintext", title: "산책 기록", isSelected: selection == .WALKRECORD) {
                        if(selection == .WALKRECORD) {
                            walkingRecordViewModel.goHome()
                        } else if(selection == . CHARACTER) {
//                            dressingRoomViewModel.is
                        } else {
                            selection = .WALKRECORD
                        }
                    }
                    
                    TabBarButton(iconName: "photo", title: "캐릭터샵", isSelected: selection == .CHARACTER) {
                    ...
                    TabBarButton(iconName: "person.fill", title: "마이 페이지", isSelected: selection == .MYPAGE) {
                        if(selection == .MYPAGE) {
