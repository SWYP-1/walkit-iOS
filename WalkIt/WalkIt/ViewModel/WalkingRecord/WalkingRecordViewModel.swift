    ...
    func loadCharacter() {
        Task {
            do {
                ...
                if let headItem = characterDto?.headImageName {
                    if(headItem.itemTag == "TOP") {
                        baseJson = await changePartItem(json: baseJson, part: "headtop", assaset: headItem.imageName)
                    } else {
                        baseJson = await changePartItem(json: baseJson, part: "headdecor", assaset: headItem.imageName)
                    }
                }
                
                if let bodyItem = characterDto?.bodyImageName {
                    baseJson = await changePartItem(json: baseJson, part: "body", assaset: bodyItem.imageName)
                }
                
                if let footItem = characterDto?.bodyImageName {
                    baseJson = await changePartItem(json: baseJson, part: "foot", assaset: footItem.imageName)
                } else {
