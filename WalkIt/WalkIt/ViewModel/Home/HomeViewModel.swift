    ...
    func loadView() {
        ...
        Task {
            ...
            if let characterDto = homeResponse.characterDto {
                ...
                UserManager.shared.level = self.level
                UserManager.shared.grade = self.grade
                UserManager.shared.characterInfo.backgroundImageName = self.backgroundImageName
                loadCharacter()
            }
        ...
        Task {
            do {
                ...
                if let feetImageName = UserDefaults.standard.string(forKey: "\(userId)feetItem") {
                    ...
                    baseJson = await setDefaultFoot(json: baseJson, grade: grade)
                }
                lottieJson = baseJson
            } catch {
